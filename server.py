...
async def chat(payload: ChatRequest):
    ...
    try:
        ...
        if re.search(r'(how many|number of|count of|total).*courses', q_lower):
            ...
            return {"answer": f"LILIT currently offers {count} courses. You can view them all at https://lms.lilit.lk/all-courses."}
        ...
        if re.search(r'\b(objective|objectives|aim|aims|goal|goals)\b', q_lower):
            ...
            if objectives_text:
                prompt = (
                    "You are a helpful assistant for LILIT LMS. The user is asking about the organization's objectives.\n"
                    "Extract ALL objectives from the following text and present them clearly as a numbered list or bullet points.\n"
                    "For EACH objective, provide:\n"
                    "- The objective title in **bold**\n"
                    "- A brief description/explanation\n\n"
                    "Make sure to include ALL objectives mentioned on the About page, not just one.\n"
                    "Provide an empty line between each objective.\n\n"
